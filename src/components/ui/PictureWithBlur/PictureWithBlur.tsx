...
import {getImageUrl, normalize} from 'src/helpers'
...
export function needToShowBlur(picture: Picture, dimensions: Dimensions) {
  ...
  const ks = [picture, dimensions].map(({width, height}) => width / height)
...
const PictureWithBlur = ({img}: Props) => {
  ...
  return needToShowBlur(img, dimensions) ? (
    <>
      <img
        ref={imgRef}
